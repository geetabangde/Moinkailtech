// EditTrfStartJob.jsx - Complete Edit TRF Component
...
export default function EditTrfStartJob() {
  ...
  const { id } = useParams(); // Get TRF ID from URL
  ...
  const getSelectedOptions = (selectedIds, options) => {
    ...
    if (typeof selectedIds === 'string') {
      return options.filter((option) => selectedIds.split(',').includes(String(option.value)));
    }
  ...
  // Convert YYYY-MM-DD to DD/MM/YYYY for server
  ...
  const fetchTrfData = async () => {
    try {
      ...
      if (response.data?.status && response.data?.data) {
        const trfData = response.data.data;
        
        // Format dates for display
        const dateInputsData = {
          date: trfData.date ? formatDateForInput(trfData.date) : todayDisplay,
          sample_received_on: trfData.sample_received_on ? formatDateForInput(trfData.sample_received_on) : "",
          wdatetime: trfData.wdatetime ? formatDateForInput(trfData.wdatetime) : "",
          dateofdispatch: trfData.dateofdispatch ? formatDateForInput(trfData.dateofdispatch) : "",
          deadline: trfData.deadline ? formatDateForInput(trfData.deadline) : "",
        };
        
        setDateInputs(dateInputsData);

        // Set form data
        setFormData({
          id: trfData.id || id,
          date: trfData.date || todayFormatted,
          sample_received_on: trfData.sample_received_on || "",
          ctype: trfData.ctype || "",
          customerid: trfData.customerid || "",
          specificpurpose: trfData.specificpurpose || "",
          ponumber: trfData.ponumber || "",
          bd: trfData.bd || "",
          promoter: trfData.promoter || "",
          priority: trfData.priority || "",
          pcharges: trfData.pcharges || "0",
          pchargestype: trfData.pchargestype || "1",
          witness: trfData.witness || "",
          wdatetime: trfData.wdatetime || "",
          wtime: trfData.wtime || "",
          wdetail: trfData.wdetail || "",
          wcharges: trfData.wcharges || "0",
          wchargestype: trfData.wchargestype || "1",
          wstatus: trfData.wstatus || "1",
          modeofreciept: trfData.modeofreciept || "1",
          localcontact: trfData.localcontact || "",
          couriername: trfData.couriername || "",
          dateofdispatch: trfData.dateofdispatch || "",
          docketno: trfData.docketno || "",
          modeofdispatch: trfData.modeofdispatch || "1",
          paymentstatus: trfData.paymentstatus || "1",
          modeofpayment: trfData.modeofpayment || "",
          detailsofpayment: trfData.detailsofpayment || "",
          paymentamount: trfData.paymentamount || "",
          certcollectiondetail: trfData.certcollectiondetail ? 
            (Array.isArray(trfData.certcollectiondetail) ? trfData.certcollectiondetail : 
            (trfData.certcollectiondetail.split ? trfData.certcollectiondetail.split(',') : [])) : [],
          additionalemail: trfData.additionalemail || "",
          certcollectionremark: trfData.certcollectionremark || "",
          returnable: trfData.returnable || "",
          documents: trfData.documents || "",
          deadline: trfData.deadline || "",
          specialrequest: trfData.specialrequest || "",
          notes: trfData.notes || "",
          letterrefno: trfData.letterrefno || "",
        });

        // Set existing files if any
        if (trfData.wupload) {
          setExistingWupload(trfData.wupload);
        }
        if (trfData.rupload) {
          setExistingRupload(trfData.rupload);
        }

        // Set visibility states based on data
        setShowPriorityCharges(trfData.priority !== "2");
        setShowWitnessDetails(trfData.witness !== "2");
        setShowCourierDetails(trfData.modeofreciept !== "1");
        setShowPaymentDetails(trfData.paymentstatus !== "2");

        // Fetch customer details if customerid exists
        if (trfData.customerid) {
          try {
            const customerRes = await axios.get(`/people/get-customer-details?id=${trfData.customerid}`);
            if (customerRes.data?.data) {
              setCustomerDetails(customerRes.data.data);
              setCustomerEmail(customerRes.data.data.email || "");
            }
          } catch (error) {
  ...
  const removeExistingFile = (fieldName) => {
    if (fieldName === "wupload") {
      setExistingWupload(null);
    } else if (fieldName === "rupload") {
      setExistingRupload(null);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Append flags for removed files
      if (!existingWupload && formData.id) {
        submitData.append("remove_wupload", "1");
      }
      if (!existingRupload && formData.id) {
        submitData.append("remove_rupload", "1");
      }

      console.log("Updating TRF:", formData.id);
      ...
      if (response.data.status) {
        toast.success("TRF entry updated successfully!");
        navigate("/dashboards/testing/trfs-starts-jobs");
      } else {
        toast.error(response.data.message || "Failed to update TRF entry");
      }
    } catch (error) {
              ...
              <div>
                ...
                {existingWupload && (
                  <div className="mb-2 p-2 bg-blue-50 rounded flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => removeExistingFile("wupload")}
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Upload new file to replace existing one</p>
              </div>
                  ...
                  <div>
                    ...
                    {existingRupload && (
                      <div className="mb-2 p-2 bg-blue-50 rounded flex items-center justify-between">
                        ...
                        <button
                          type="button"
                          onClick={() => removeExistingFile("rupload")}
                          className="ml-2 text-red-600 hover:text-red-800"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
